...
class Nallocate(object):
    ...
    def N_distrib(grid,tabRay):
        ...
        PAR_voxmean = np.zeros(1300) #tableau contenant la valeur moyenne de PAR cumule journalier par voxel
## Defini a 1300 = nb de voxel pour une grille de 10 x 10 x 13 voxels => changer ce nombre  sichangement de grille!!!
...
##            print 'PARi2', ik, PARi2[n_celvox]
            PAR_voxmean[ik]=PARvox #Ajout de la valeur a un vecteur pour le PAR total
...
##        print "np.size(grid.n_detailed)2",np.size(grid.n_detailed)
        final = np.concatenate((grid.n_detailed[0],PAR_voxmean))
